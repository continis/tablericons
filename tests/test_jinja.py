...
def test_success_icon_path_attr():
    env = make_environment('{{ lucide("x", stroke_linecap="butt") }}')
    template = env.get_template("index")
...
def test_success_icon_complete():
    env = make_environment(
        '{{ lucide("x", size=48, class="h-4 w-4", ' + 'data_test="a < 2") }}'
    )
    template = env.get_template("index")
...
def test_success_icon_size_none():
    env = make_environment('{{ lucide("x", size=None) }}')
    template = env.get_template("index")
